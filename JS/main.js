...
function initUI() {
    ...
    const offsetMode = document.getElementById('Offset-Button');
    ...
    const OffsetModeStorage = window.localStorage.getItem('OffsetMode');
    const SamplingCountStorage = window.localStorage.getItem('SamplingCount');

    // Undefined = false

    if(toInt(SamplingCountStorage)!=null){
        FFT.N = toInt(SamplingCountStorage);
    ...
    if (toBoolean(OffsetModeStorage)) {
        FFT.offset = toBoolean(OffsetModeStorage);
        offsetMode.checked = toBoolean(OffsetModeStorage);
    }
    ...
    offsetMode.addEventListener('change', () => {
        FFT.offset = offsetMode.checked;

        window.localStorage.setItem('OffsetMode', offsetMode.checked);
    });

    samplingCount.addEventListener('change', () => {
        if (Number.isInteger(Math.log2(samplingCount.value))) {
            FFT.N = samplingCount.value;
        window.localStorage.setItem('SamplingCount', samplingCount.value);
